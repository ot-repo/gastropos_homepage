...
import { ArrowRight, Utensils, ShoppingBag, Scissors, Sparkles, Brain, TrendingUp } from "lucide-react";
...
export function Hero() {
  ...
  return (
    <section className="relative -mt-20 overflow-hidden pt-36 pb-32 bg-hero-blue">
      <div aria-hidden className="absolute inset-0 bg-hero-grid [mask-image:radial-gradient(ellipse_70%_60%_at_50%_30%,black,transparent)]" />
      ...
      <div className="relative mx-auto max-w-7xl px-6 text-center text-white">
        ...
        >
          ...
          <div className="mt-9 flex flex-wrap items-center justify-center gap-3">
            <Link to="/demo" className="group inline-flex items-center gap-2 rounded-full bg-white px-7 py-3.5 font-semibold text-accent shadow-[0_10px_30px_-10px_rgba(15,23,42,0.4)] transition-transform hover:-translate-y-0.5">
              {copy.cta1} <ArrowRight className="size-4 transition-transform group-hover:translate-x-0.5" />
            </Link>
            <Link to="/demo" className="inline-flex rounded-full border border-white/30 bg-white/10 px-7 py-3.5 font-semibold text-white backdrop-blur transition-all hover:bg-white/20">
              {copy.cta2}
            </Link>
          </div>

          <div className="mt-10 flex flex-wrap items-center justify-center gap-2">
            <AiPill icon={<Brain className="size-3.5" />} label={lang === "de" ? "Nachfrageprognose" : "Demand forecasting"} />
            <AiPill icon={<Sparkles className="size-3.5" />} label={lang === "de" ? "Menü-Insights" : "Menu insights"} />
            <AiPill icon={<TrendingUp className="size-3.5" />} label={lang === "de" ? "Dynamische Preise" : "Dynamic pricing"} />
          </div>

          <div className="mt-8 flex flex-col items-center gap-4">
            <p className="font-mono text-[11px] uppercase tracking-widest text-white/70">{copy.builtFor}</p>
            <div className="flex flex-wrap items-center justify-center gap-3">
              <PlatformPill icon={<Utensils className="size-4" />} label={lang === "de" ? "Gastronomie" : "Hospitality"} />
              <PlatformPill icon={<ShoppingBag className="size-4" />} label="Retail" />
              <PlatformPill icon={<Scissors className="size-4" />} label={lang === "de" ? "Dienstleister" : "Services"} />
            </div>
          </div>
        </motion.div>
        ...
        <motion.div
          initial={{ opacity: 0, y: 40 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.9, delay: 0.2, ease: [0.16, 1, 0.3, 1] }}
        ...
        >
            ...
            <div className="flex h-[480px] w-full gap-3 overflow-hidden rounded-lg bg-background">
              ...
              <div className="flex flex-1 flex-col p-6">
                <div className="flex items-center justify-between border-b border-border pb-4">
                  <div className="text-left">
                    <span className="block font-mono text-[10px] uppercase tracking-tighter text-muted-foreground">Active Table</span>
                ...
                <div className="grid grid-cols-3 gap-3 py-6">
                  {["Espresso", "Riesling", "Seabass", "Tiramisu", "Cappuccino", "Pasta"].map((item, i) => (
                    <motion.div
                      key={item}
                    ...
                    >
                      ...
                      <p className="mt-1 font-mono text-[10px] text-muted-foreground">€{(i + 3) * 3.5}.00</p>
                    </motion.div>
                  ))}
                </div>
                <div className="mt-auto flex items-center justify-between rounded-xl bg-foreground p-4 text-primary-foreground">
                  <div>
                    <p className="font-mono text-[10px] uppercase tracking-widest opacity-60">Total</p>
                    <p className="font-display text-xl font-bold">€84.50</p>
                  </div>
                  <button className="rounded-md bg-accent px-4 py-2 text-xs font-semibold">Tap to Pay</button>
                </div>
              </div>
              <div className="hidden md:flex w-64 flex-col border-l border-border bg-surface p-5">
                <div className="flex justify-between font-mono text-[10px] uppercase tracking-widest text-muted-foreground mb-4">
                  <span>DATEV Export</span><span>#0842-X</span>
                </div>
          ...
          >
            <div className="flex items-center gap-3">
              <div className="size-10 rounded-full bg-accent-soft grid place-items-center text-accent font-bold">↑</div>
              <div className="text-left">
...
function RowLine({ label, value, bold, muted }: { label: string; value: string; bold?: boolean; muted?: boolean }) {
  return (
    <div className={`flex justify-between ${bold ? "font-bold text-foreground" : muted ? "text-muted-foreground" : "text-foreground"}`}>
