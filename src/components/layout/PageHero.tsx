...
export function PageHero({
  ...
  return (
    <section className="relative overflow-hidden pt-20 pb-16 border-b border-border">
      <div
        aria-hidden
        className="absolute inset-0 -z-10 bg-grid opacity-30 [mask-image:radial-gradient(ellipse_60%_50%_at_50%_0%,black,transparent)]"
      />
      <div className="mx-auto max-w-4xl px-6 text-center">
        {eyebrow && (
          <p className="font-mono text-[11px] uppercase tracking-widest text-accent">{eyebrow}</p>
        )}
        <h1 className="mt-4 font-display text-5xl font-extrabold tracking-tight md:text-6xl text-balance">
          {title}
        </h1>
        ...
        <div className="mt-8 flex flex-wrap justify-center gap-3">
          <Link
            to={ctaTo}
            className="rounded-full bg-accent px-6 py-3 text-sm font-semibold text-accent-foreground shadow-elegant"
          >
            {t.common.bookDemo}
          </Link>
          <Link
            to="/pricing"
            className="rounded-full border border-border bg-surface px-6 py-3 text-sm font-semibold"
          >
            {t.nav.pricing}
          </Link>
        </div>
