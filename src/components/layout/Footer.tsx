      ...
      <div className="mx-auto max-w-7xl px-6">
        <div className="grid gap-12 lg:grid-cols-6">
          <div className="col-span-2">
            ...
            <p className="mt-6 max-w-xs text-sm text-muted-foreground leading-relaxed">{t.footer.tagline}</p>
            <div className="mt-8 flex items-center gap-3 text-[11px] font-mono uppercase tracking-widest text-muted-foreground">
              <span className="flex items-center gap-1.5"><span className="size-1.5 rounded-full bg-success" />TSE</span>
              <span className="flex items-center gap-1.5"><span className="size-1.5 rounded-full bg-success" />DSFinV-K</span>
              <span className="flex items-center gap-1.5"><span className="size-1.5 rounded-full bg-success" />GoBD</span>
              <span className="flex items-center gap-1.5"><span className="size-1.5 rounded-full bg-success" />GDPR</span>
            </div>
          </div>
          <FooterCol title={t.footer.product}>
            {productLinks.slice(0, 8).map((l) => (
              <Link key={l.to} to={l.to as any} className="hover:text-accent">{lang === "de" ? l.de : l.en}</Link>
            ))}
          </FooterCol>
          ...
          <FooterCol title={t.footer.company}>
            <Link to="/about" className="hover:text-accent">{lang === "de" ? "Über uns" : "About"}</Link>
            <Link to="/contact" className="hover:text-accent">{lang === "de" ? "Kontakt" : "Contact"}</Link>
            
            <Link to="/partners" className="hover:text-accent">Partners</Link>
            <Link to="/affiliate" className="hover:text-accent">{lang === "de" ? "Affiliate" : "Affiliate"}</Link>
            
            <Link to="/pricing" className="hover:text-accent">{t.nav.pricing}</Link>
            <Link to="/compare" className="hover:text-accent">{t.nav.compare}</Link>
          </FooterCol>
          <FooterCol title={t.footer.legal}>
            <Link to="/legal/privacy" className="hover:text-accent">{lang === "de" ? "Datenschutz" : "Privacy"}</Link>
